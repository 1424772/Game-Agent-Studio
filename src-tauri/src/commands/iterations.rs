...
fn requires_approval(pt: &ProposalType) -> bool {
    matches!(
        pt,
        ProposalType::CodeImprovement
            | ProposalType::PromptImprovement
            | ProposalType::SafetyEnhancement
            | ProposalType::ExportTemplateFix
            | ProposalType::DataModelRefinement
    )
}

fn log_proposal_event_tx(
    tx: &rusqlite::Transaction, event_type: &str, proposal_id: &str,
    ...
    let payload = crate::models::sanitize_error(
        serde_json::json!({
...
pub fn create_improvement_proposal(
    ...
    let evidence = validate_not_empty("evidence", evidence.as_deref().unwrap_or(""))?;
    let risk = risk_level.ok_or_else(|| "risk_level is required".to_string())?;
    let risk = validate_risk_level(&risk)?;
    let target_area = validate_not_empty("target_area", target_area.as_deref().unwrap_or(""))?;
    let proposed_change = validate_not_empty("proposed_change", proposed_change.as_deref().unwrap_or(""))?;
    ...
    log_proposal_event_tx(&tx, events::EVENT_PROPOSAL_CREATED, &id,
...
fn read_proposal(row: &rusqlite::Row<'_>) -> rusqlite::Result<ImprovementProposal> {
    ...
    Ok(ImprovementProposal {
        id: row.get(0)?, proposal_type: parse_proposal_type(&pt),
        summary: row.get(2)?, evidence: row.get(3).ok(), risk_level: row.get(4).ok(),
        status, requires_human_approval: rha != 0, target_area: row.get(7).ok(),
        proposed_change: row.get(8).ok(), created_at: row.get(9)?, reviewed_at: row.get(10).ok(),
    })
}
...
pub fn review_improvement_proposal(
    ...
    log_proposal_event_tx(&tx, events::EVENT_PROPOSAL_REVIEWED, &proposal_id,
...
mod tests {
    ...
    #[test]
    fn approval_required_for_dangerous_types() {
