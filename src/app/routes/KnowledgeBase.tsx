  ...
  return (
    <div className="page">
      ...
      <div className="card form-card"><h3 className="form-title">{t.knowledge.search}</h3><div style={{display:'flex',gap:'0.5rem'}}><input className="form-input" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder={t.knowledge.searchPlaceholder} onKeyDown={e=>e.key==='Enter'&&handleSearch()}/><button className="btn btn-primary" onClick={handleSearch}>{t.knowledge.search}</button></div>
        {searchResults && <div style={{marginTop:'1rem'}}><p className="text-sm text-secondary">{searchResults.excerpts.length} {t.knowledge.results} / {searchResults.run.duration_ms}{t.knowledge.ms}</p>{searchResults.excerpts.map(h=><div key={h.id} style={{padding:'0.5rem',borderBottom:'1px solid var(--border)'}}><div style={{display:'flex',justifyContent:'space-between'}}><span><strong>{h.doc_title}</strong> <span className="badge">{h.doc_type}</span></span><span className="text-secondary">Score: {h.score.toFixed(1)} | Rank: {h.rank}</span></div><pre style={{fontSize:'0.8rem',maxHeight:'80px',overflow:'auto',marginTop:'0.25rem'}}>{h.chunk_excerpt}</pre><div className="text-sm text-secondary">{h.source && <span>Source: {h.source}</span>}{h.provenance && <span style={{marginLeft:'1rem'}}>{h.provenance.slice(0,80)}</span>}</div></div>)}</div>}
      </div>
